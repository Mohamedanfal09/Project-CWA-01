...
export default function HomePage() {
  ...
  const [contents, setContents] = useState(["Install VSCode", "Install Chrome"]);
  ...
  const generateHtml = () => {
    ...
    tabs.forEach((tab, index) => {
      ...
      tabsContent += `<div id="tab${index}" style="display:${index===0?"block":"none"};padding:10px;border:1px solid #ccc;margin-top:5px;">${contents[index]}</div>\n`;
    });
...
function showTab(index) {
  ...
  return (
    <div style={containerStyle}>
      <h1 style={titleStyle}>HTML5 Tabs Generator</h1>
      ...
      <button style={buttonStyle} onClick={addTab}>Add Step</button>
      <button style={buttonStyle} onClick={generateHtml}>Generate HTML</button>
